...
def check_image(image)
  ...
  res = Net::HTTP.get_response(URI("https://passkeys-api.2fa.directory/icons/#{image[0]}/#{image}"), headers)
  res.code.eql? '200'
end

path = ENV['LOCAL_2FA_PATH']
response = path ? File.read("#{path}/private/all.json") : Net::HTTP.get(URI('https://passkeys-api.2fa.directory/private/all.json'))
entries = {}
